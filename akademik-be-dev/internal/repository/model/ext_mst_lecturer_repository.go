...
type MstLecturerRepository struct {
	repository.Repository[model.MstLecturer]
	log             *logrus.Logger
	cacheRepository cached.CacheRepository
}

func NewMstLecturerRepository(log *logrus.Logger,
	cacheRepository cached.CacheRepository,
) *MstLecturerRepository {
	return &MstLecturerRepository{
		log:             log,
...
/* Read */
func (r *MstLecturerRepository) GetAllWithCountByProgramHeadID(db *gorm.DB, ctx context.Context, pg pageable.PageableRequestInterface) (T []model.MstLecturer, count int64, err error) {
	user := ctx.Value("x-user-claims").(*auth.UserClaimsSpesifikRole)
	query := db.WithContext(ctx).Model(&model.MstLecturer{}).
		Where("program_head_user_id = ? AND (deleted_at IS NULL OR deleted_at = 0)", user.ID)

	search := pg.GetDefaultSearch()
	if search != "" {
		query = query.Where("name ILIKE ? OR nidn ILIKE ?", "%"+search+"%", "%"+search+"%")
	}
