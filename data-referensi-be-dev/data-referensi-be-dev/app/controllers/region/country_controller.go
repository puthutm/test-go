...
func GetCountries(c *fiber.Ctx) error {
	...
	query_results, total, err := models.GetCountries(filter, sortBy, sortDirection, page, pageSize)
	if err != nil {
...
func SearchCountries(c *fiber.Ctx) error {
	...
	query_results, err := models.SearchCountries(filter, sortBy, sortDirection, page, pageSize)
	if err != nil {
		return handlers.SendFailed(c, fiber.StatusOK, nil, helpers.GenerateRM("get", false))
	}
...
func ExportCountries(c *fiber.Ctx) error {
	return handlers.ControllerExport(c, "Countries", models.ExportCountries)
}

func GetCountry(c *fiber.Ctx) error {
	id := c.Params("id")
	query_result, err := models.GetCountry(id)
	if err != nil {
...
func CreateCountry(c *fiber.Ctx) error {
	...
	id, err := helpers.EnsureUUID(&models.MstCountry{})
	if err != nil {
		return err
	}

	err = models.CreateCountry(id, req.Name, req.PhoneCode, req.IconFlagPath)
	if err != nil {
		return handlers.ErrorQuery(c, err)
	}
...
func ImportCountries(c *fiber.Ctx) error {
	return handlers.ControllerImport(c, models.ImportCountries)
}
...
func GetTrashCountries(c *fiber.Ctx) error {
	filter, sortBy, sortDirection, page, pageSize := handlers.ControllerQueryParams(c)

	query_results, total, err := models.GetTrashCountries(filter, sortBy, sortDirection, page, pageSize)
	if err != nil {
		return handlers.SendFailed(c, fiber.StatusOK, nil, helpers.GenerateRM("get", false))
	}
