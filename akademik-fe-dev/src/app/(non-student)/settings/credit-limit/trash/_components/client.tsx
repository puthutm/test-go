...
import { useCreditLimitTrashColumn } from "./column";

const CreditLimitTrashClient = ({
  dataTrashSksLimit,
}: {
  dataTrashSksLimit: ApiResponse<PaginationData<ISksLimit>>;
}) => {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const params = new URLSearchParams(searchParams);
  const { columns } = useCreditLimitTrashColumn();
        ...
        <Card className="p-0">
          ...
          <CardBody>
            <Col className="table-responsive" sm={12}>
              <DataTables
                columns={columns}
                data={dataTrashSksLimit?.data}
                pageCount={dataTrashSksLimit?.data?.metadata.total_page}
                pagination={dataTrashSksLimit?.data?.metadata}
                setPagination={handlePagination}
                total={dataTrashSksLimit?.data?.metadata.total_data}
              />
            </Col>
          </CardBody>
        </Card>
...
export default CreditLimitTrashClient;
