...
export const FormSubject = ({ isEdit, isDetail, data }: FormSubjectProps) => {
  ...
  const curriculumYearId = params?.curriculumYearId as string;
  ...
  const studyProgramId = watch("study_program_id");
  ...
  const { data: lecturer, isLoading: isLoadingLecturer } = useSearchLecturer({
    page: 1,
    limit: 200,
    study_program_id: studyProgramId?.value || "",
    search: "",
  });
  ...
  const onSubmit = async (data: FormSubjectSchemaType) => {
    try {
      const response = isEdit
        ? await editSubject(subjectId, data)
      ...
      return router.push(
        `/settings/curriculum9year/${curriculumYearId}/subject`
      );
    } catch (error: any) {
      setModalConfirmationState((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    // set curriculum year based on curriculumYearId params
    if (curriculumYearOptions) {
      const selectedCurriculumYear = curriculumYearOptions?.find(
        (option) => option.value === curriculumYearId
      );

      setValue("curriculum_year_id", {
        label: selectedCurriculumYear?.label as string,
        value: selectedCurriculumYear?.value as string,
      });
    }
  }, [curriculumYearId, curriculumYearOptions, setValue]);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} autoComplete="off">
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <SelectComponent
                        options={lecturerOptions as OptionType[]}
                        isLoading={isLoadingLecturer}
                        placeholder="Pilih Dosen Pengembang RPS"
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <SelectComponent
                        options={lecturerOptions as OptionType[]}
                        isLoading={isLoadingLecturer}
                        placeholder="Pilih Koord. Pengampu MK"
      ...
      <div className="d-flex justify-content-between mt-3">
        <Link href={`/settings/curriculum-year/${curriculumYearId}/subject`}>
          <button
            type="button"
            className={
