...
import ModalFilterSubjects from "./modal-filter-subject";
import DataTables from "@/components/ui/datatable";

// import hooks
import useColumnLecturerSubject from "./column-definintion-subject";
...
interface IPropsParamsPageSubject {
  dataSubject: ApiResponse<PaginationData<ILectureSubjects>>;
}

function PageSubjectLecturer({ dataSubject }: IPropsParamsPageSubject) {
  ...
  const [showModalFilter, setShowModalFilter] =
    useState<IModalManipulationFilterSubjects>({
      status: false,
      title: "Filter",
    });

  //! column
  const { columns } = useColumnLecturerSubject();
...
>>>>>>> Feat/lecturer
                ...
                <Button
                  ...
                  color="transparent"
                  // disabled={dataSubjectisFetchingSubject}
                  onClick={() => {
                    handleSearch();
                  }}
          ...
          <CardBody className="p-0 m-0">
            <section className="table-responsive">
              <DataTables
                ...
                pageCount={dataSubject?.data.metadata?.total_page as number}
                pagination={dataSubject.data.metadata}
                setPagination={handlePagination}
                isLoading={false}
                total={dataSubject?.data.metadata?.total_data as number}
              />
            </section>
          </CardBody>
        </Card>
...
export default PageSubjectLecturer;
