...
import { updateClassForProgramHead } from "@/services/api/curriculum/academic-period/class/update-class";
...
export const FormClassDetail = ({
  ...
  const {
    ...
    defaultValues: {
      ...
      subject_id: {
        label: data.data.subject_name_id,
        value: data?.data.subject_id,
      },
      academic_period_id: data?.data?.academic_periode_id || "",
      curriculum_year_id: {
  ...
  const onSubmit = async (payload: FormClassForProgramHeadType) => {
    try {
      const response = await updateClassForProgramHead(classId, payload);
