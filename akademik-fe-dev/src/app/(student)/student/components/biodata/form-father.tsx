...
export const FormFather = ({
  ...
  father: ApiResponse<ParentStudent>;
}) => {
  ...
  const handleSetFormValue = useCallback(() => {
    setValue("name", father?.data?.name);
    setValue("nik", father?.data?.nik);
    setValue("email", (father?.data?.email as string) || undefined);
    setValue("phone", (father?.data?.phone as string) || "");
    setValue("phone2", (father?.data?.phone2 as string) || "");
    if (father?.data?.educational_level_id) {
      setValue("education_level_id", {
        label: father?.data?.educational_level_name as string,
        value: father?.data?.educational_level_id,
      });
    }
    if (father?.data?.job_id) {
      setValue("job_id", {
        label: father?.data?.job_name as string,
        value: father?.data?.job_id,
      });
    }
    if (father?.data?.birth_place_id) {
      setValue("birth_place_id", {
        label: father?.data?.birth_place_name as string,
        value: father?.data?.birth_place_id,
      });
    }
    if (father?.data?.status_kinship) {
      const selectedStatusKinship = STATUS_KINSHIP_OPTIONS?.find(
        (kinship) => kinship.value === father?.data?.status_kinship
      );
    ...
    if (father?.data?.life_status) {
      const selectedStatusKinship = LIFE_STATUS_OPTIONS?.find(
        (status) => status.value === father?.data?.life_status
      );
    ...
    setValue("birth_date", [new Date(father?.data?.birth_date as string)]);
    setValue("address", (father?.data?.address as string) || "");
    setValue("income", (father?.data?.income?.toString() as string) || "");
  }, [father, setValue]);

  const onSubmit = async (payload: FormParentSchemaType) => {
    try {
      const response = await updateParent({ parentType: "father", payload });
  ...
  }, [father, handleSetFormValue]);

  if (father?.error) {
    return <h1>{father.message}</h1>;
  }
  return (
    <>
