...
export const getAllClassByAcademicPeriodIdForProgramHead = async (
  queryParam: QueryParamClass
): Promise<ApiResponse<PaginationData<Class[]>>> => {
  const params = new URLSearchParams();

  console.log(queryParam.academic_periode_id);
  ...
  try {
    const response = await fetchApi(
      `/program-head/curriculum/academic-period/classes?${params.toString()}`
    );
