...
export const createSubject = async (payload: FormSubjectSchemaType) => {
  const reqBody = {
  ...
  try {
    const response = await fetchApi("/academic/setting/subjects", {
      method: "POST",
    ...
    revalidatePath(
      `/settings/curriculum-year/[curriculumYearId]/subjects`,
      "page"
    );
