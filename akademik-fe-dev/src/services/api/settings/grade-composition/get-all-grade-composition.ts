...
export const getAllGradeCompositions = async (
  ...
  try {
    const xhr = await fetchApi(
      `/academic/setting/value-compositions?${params.toString()}`,
      {
        method: "GET",
      }
    );
    ...
    throw new Error(err.message || "gagal get data komposisi nilai");
  }
};
