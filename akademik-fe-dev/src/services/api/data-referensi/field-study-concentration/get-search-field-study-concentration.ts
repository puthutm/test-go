...
export const getSearchFieldStudyConcentration = async (
  queryParam: QueryParamDataRefensi
): Promise<ApiResponse<FieldStudyConcentration[]>> => {
  ...
  try {
    const response = await fetchApiDatareferensi(
      `/academic/field-study-concentrations/search?${params.toString()}`
    );
